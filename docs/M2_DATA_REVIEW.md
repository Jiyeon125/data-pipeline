...
- 원본 사업-연도 9,366행 중 broad 6,177행, core 6,130행을 용도에 맞게 분리했습니다.
- 기존 strict v1은 core의 37.0%만 남겼지만, ranking v2는 행을 유지하고 변수별 적격성을 제한해 core 6,130행을 보존했습니다.
- 프로그램 연결은 PARTIAL 5행, UNMATCHED 127행이며 이들의 집행률은 전체값처럼 해석하지 않습니다.
- 잠정 기준으로 연말 집중 신호는 832개 사업-연도, 반복 집행설명필요 사업은 131개입니다. 이는 낭비·실패 판정이 아니라 원문 확인 순서입니다.
...
- 결산 연결: 8,407행
- BLOCKING/NON_BLOCKING/INFORMATIONAL: 46/1,945/725행
- 재정수단 UNKNOWN: 3,100행
- 관계 수기검토 후보: 2,501행
- 집행률 100% 초과: 0행
...
broad 본예산 합계는 1,103.5조 원, core 결산 지출 합계는 1,546.1조 원입니다.
...
회계유형-연도 집계는 16개 셀입니다. 일반·특별회계는 확인된 예산현액을 분모로 사용했습니다. 기금은 지출계획현액과 원천 필드의 공식 대응이 확인되지 않아 집행률 분포에서 제외하고 데이터 검증 대상으로 유지했습니다.
...
월별 패턴 최종 적격 사업-연도는 2,470행입니다. 잠정 연말 집중 기준은 4분기 비중 40% 이상 또는 12월 비중 20% 이상입니다.
...
단일 세부사업 본예산 비중이 50% 이상인 프로그램-연도는 206행입니다.
...
strict v1의 core 본예산 포함률은 10.0%, ranking v2는 100.0%입니다. 고용노동부 기금 2022~2025년과 과학기술정보통신부 기금 2024년의 대표성 제한 경고를 유지합니다.

## 10. 주요 발견

- 반복 집행설명필요 사업 131개는 사례 원문 확인 후보입니다.
- 프로그램 내부 단일사업 의존 후보 206개는 구조 설명이 필요합니다.
...
| 관계 | n | 결측 | Spearman |
|---|---:|---:|---:|
| current_budget_vs_settlement | 6,130 | 0 | 0.996 |
| project_size_vs_execution_rate | 4,621 | 1,509 | 0.100 |
| previous_execution_vs_current_budget_change | 1,797 | 4,333 | 0.008 |
| repeated_under_90_count_vs_budget_change | 3,049 | 3,081 | -0.036 |
