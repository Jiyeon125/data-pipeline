...
> 작성일: 2026-07-28  
> 상태: 중기부 가린 골드셋 12요청 실행·검증 완료, 자동 승격 금지
...
2026-07-31 근거 승인 규칙을 적용한 실제 dry-run에서는 424행 중 346행을
출처·페이지·보고서 원문 근거로 승인하고 기존 사람 검수 49행을 보존했습니다.
사람 판단이 필요한 29행만 별도 큐에 남았으므로 현재 수기 기준선의 LLM 요청과
예상 API 비용은 각각 0건, `$0`입니다.

LLM 호출 하네스는 삭제하지 않습니다. 향후 새 성과계획서·성과보고서를 처음
구조화하거나, 수기 골드셋 일부를 가린 모델 파일럿을 할 때만 사용합니다.
...
수기값 대조, 근거 페이지와 상태 저장을 수행합니다. 현재 29행은 출처나 지표
대응을 사람이 판단해야 하므로 LLM에 보내지 않습니다.
...
### 4.1 2026-07-31 현재 수기 기준선 dry-run
...
외부 호출 잠금 `api_execution_allowed=false`는 API 키 유무와 무관하게 사용자가
실행별 비용 상한을 명시적으로 승인할 때까지 유지합니다.

### 4.2 중기부 가린 골드셋 파일럿 dry-run

수기 정답과 로컬 추출 후보를 요청에서 제거하고 원문 파일·페이지·근거문장과
지표 위치만 남긴 실제 파일럿을 생성했습니다.
...
선정 요청: 12
선정 지표 행: 34
연도별 행: 2022년 8, 2023년 19, 2024년 7
입력 추정: 25,682 tokens
기대 출력: 5,400 tokens
승인상한 출력: 21,600 tokens
GPT-5.6 Luna 기대 Batch 비용: 약 $0.0058
GPT-5.6 Luna 보수적 Batch 상한: 약 $0.0156
외부 API 호출: 0건(이 절은 제출 전 dry-run 기록)
```

파일럿은 알려진 프로그램·지표를 원문 안에서 찾도록 위치 단서를 제공합니다.
따라서 값·근거 구조화 정확도, 누락, 문서에 없는 값 생성 여부는 측정할 수 있지만,
PDF 전체에서 지표 행을 처음부터 발견하는 재현율은 측정할 수 없습니다. 전자는 이번
파일럿의 승인 기준으로 사용하고 후자는 다음 단계의 문서 단위 홀드아웃으로 분리합니다.
...
검증 통과 요청: 11/12
엄격 검증 지표: 23/34
로컬 보수 복구 포함: 32/34, 94.1%
중복 원문 충돌 제외 적용 가능: 30/34, 88.2%
남은 최소 재시도: 1요청·2지표
```

유효 응답 중 수기 정답이 있는 구간에서는 지표명 100%, 단위 91.3%, 계획 목표
95.7%, 실적과 공식 달성률 100%가 일치했습니다. 그러나 최대 출력토큰 초과
1요청·6지표와 근거 인용 불일치 1요청·5지표가 있어 전체 자동화 성능으로 해석할 수
없습니다. 수기 결측에서 찾은 원문 근거값 8건은 사람 검수 전 보완 후보로만 둡니다.

비용·순편익 산식과 발표용 해석은 `docs/LLM_PILOT_COST_BENEFIT.md`에 분리했습니다.
...
확대합니다. 이 방식이면 모델 선택 실패가 전체 신규 문서의 재호출로 이어지지
않습니다.
...
1. **현재 기준선 근거 승인 — 완료**: 424행을 근거 승인 346행, 기존 사람 검수
   49행, 사람 판단 필요 29행으로 분리했습니다.
2. **잔여 사람 검수**: 29행만 원문에서 확인하고 별도 감사 CSV에 기록합니다.
3. **가린 골드셋 파일럿 — 로컬 준비 완료**: 중기부 12요청·34지표에서 수기
   정답을 숨겼습니다. 실제 응답의 추출·근거 정확도 측정은 API 승인 후 진행합니다.
4. **라우팅 검증**: 저가 모델 단독, 저가+상위 재검수, 상위 모델 단독을 같은
   표본에서 비교합니다.
5. **나머지 부처 확대**: 원본 별첨과 수기 행이 확보된 뒤 같은 명령으로 실행합니다.

현재 기준선에는 외부 호출이 필요하지 않습니다. 중기부 파일럿은 API 키까지
인식했지만 `api_execution_allowed=false`를 유지합니다. 사용자의 명시 승인과
실행별 `$0.02` 상한 확인 뒤 12개 Batch 요청만 제출합니다.
...
현재 준비한 중기부 파일럿은 다음 두 데이터 경계를 강제합니다.

```text
로컬 비교 전용: candidate_rows.csv의 수기 정답·로컬 값 후보·검수 상태
API 전송 대상: pilot_requests.jsonl의 원문 근거·파일·페이지·지표 위치
```

두 파일을 분리하고 요청 JSON에 수기 정답 키가 있으면 생성을 실패시킵니다.
응답을 받으면 필드 정확도·누락률·비근거 비null 비율·근거 페이지 정확도를 먼저
산출하며, 결과를 수기 기준선에 자동 반영하지 않습니다.
...
## 19. 수기 목록 없는 전체 PDF 탐색 파일럿

`fiscal-performance discover-performance-pdfs`는 수기 지표명이나 정답 행을 입력으로
쓰지 않고 4개 부처의 2022~2024년 계획서·보고서 24개를 처음부터 훑습니다. 골드셋은
추출이 끝난 뒤 발견률과 값 일치율을 채점할 때만 읽습니다.

```text
PDF 전체 페이지 로컬 표 탐색
→ 프로그램목표 번호·지표명·연도 열·값 추출
→ 근거가 완전한 행 LOCAL_CONFIRMED
→ 계층·값이 불완전한 행 LLM_REVIEW_REQUIRED
→ 글꼴 매핑이 깨진 문서 OCR_REQUIRED
→ 골드셋 사후평가
```

2026-08-01 기준 실제 결과는 다음과 같습니다.
...
| 발견된 계획 목표 정확도 | 98.5% |
| 발견된 보고서 실적 정확도 | 99.4% |
| 발견된 보고서 달성률 정확도 | 98.8% |
| 외부 API 호출 | 0회 |

발견되지 않은 행을 0이나 추정값으로 채우지 않습니다. 특히 화면에는 정상 한글로
보여도 PDF 내부 글꼴 매핑이 깨진 보건복지부 2024 계획서처럼, 텍스트 추출 결과가
무의미한 문서는 로컬 OCR 또는 비전 LLM 대상으로 보냅니다. 로컬 Tesseract를
200·300dpi로 시험한 결과 제목은 복원했지만 표의 지표명·목표치 셀은 안정적으로
복원하지 못했습니다.

따라서 현재 파일럿은 `PDF 투입 → 안전한 자동 통과 → 실패 문서 자동 라우팅`까지
무인으로 수행합니다. 모든 문서를 사람 없이 최종 확정하는 단계는 아직 통과하지
않았습니다. 다음 단계는 전체 PDF를 LLM에 보내는 방식이 아니라, 로컬 추출에 실패한
페이지 이미지만 비전 LLM에 보내고 JSON Schema·원문 존재 검증을 통과한 값만
승격하는 선택적 경로입니다.
