...
집행률 분석 적격·100% 초과 제외 모집단은 6,198행입니다. 80% 미만은 270행(4.4%), 90% 미만은 532행(8.6%)입니다. 80%와 90%는 분포가 자동으로 발견한 절단점이 아니라, 탐지 강도를 두 단계로 설명하기 위한 잠정 운영 기준입니다.
...
80% 기준은 고유 사업 179개와 예산현액 2.0%를 탐지합니다. 90% 기준은 고유 사업 303개와 예산현액 11.9%를 탐지합니다. 두 값을 합쳐 하나의 정책 판정으로 쓰지 않고 강한 신호와 주의 신호로 나누는 이유입니다.
...
89%에서 90%로 이동할 때 새로 포함되는 행은 41개지만 예산현액 비중은 8.8%p 증가합니다. 90% 기준의 금액 영향은 소수 대규모 사업에 민감하므로 해당 구간 원자료를 별도 보존했습니다.

| 부처 | 프로그램 | 세부사업 | 연도 | 집행률 | 예산현액 |
|---|---|---|---:|---:|---:|
| 행정안전부 | 지방교부세 | 보통교부세 | 2023 | 89.2% | 66.64조원 |
| 보건복지부 | 여유자금운용 | 국채매입 | 2025 | 89.4% | 39.20조원 |
...
|---|---|---:|---:|---:|
| ACCOUNT_TYPE | RESPONSIBLE_OPERATION_ACCOUNT | 182 | 23.1% | 5.30x |
| FISCAL_INSTRUMENT | GUARANTEE | 48 | 22.9% | 5.26x |
| DATA_QUALITY | RESTRICTED | 259 | 13.1% | 3.01x |
| FISCAL_INSTRUMENT | LOAN | 60 | 11.7% | 2.68x |
| FISCAL_INSTRUMENT | FACILITY | 141 | 10.6% | 2.44x |
| FISCAL_INSTRUMENT | OPERATION | 690 | 10.3% | 2.36x |

### 90% 기준에서 우선 확인할 집단

| 차원 | 집단 | 적격 행 | 탐지율 | 전체 대비 배수 |
|---|---|---:|---:|---:|
| ACCOUNT_TYPE | RESPONSIBLE_OPERATION_ACCOUNT | 182 | 44.5% | 5.19x |
| DATA_QUALITY | RESTRICTED | 259 | 36.7% | 4.27x |
| FISCAL_INSTRUMENT | GUARANTEE | 48 | 29.2% | 3.40x |
| FISCAL_INSTRUMENT | FACILITY | 141 | 20.6% | 2.40x |
| FISCAL_INSTRUMENT | OPERATION | 690 | 19.7% | 2.30x |
| DATA_QUALITY | NON_BLOCKING | 398 | 18.3% | 2.14x |
...
검증된 월별 패턴에서 4분기 기준만 충족한 행은 78행, 12월 기준만 충족한 행은 207행, 두 기준을 모두 충족한 행은 176행입니다. 고정 기준은 충족하지 않지만 보수적 P90인 행은 91행이고, 고정 기준과 P90을 모두 충족한 행은 335행입니다.
...
동일 신호가 2회 이상이면서 유효연도의 50% 이상인 사업은 집행률 90% 미만 기준 139개, 고정 연말집중 기준 112개입니다.
...
고정 연말집중 반복 후보 112개는 현재 모두 유효 월별 관측 2년에서 연속 2회 발생한 사업입니다. 월별 관측 가능한 연도가 제한되어 있으므로 집행률 반복보다 증거가 약하며, 발표에서는 '2년 연속 관찰'로 표현해야 합니다.
...
|---|---|---:|---:|---:|---|---|
| 집행률 80% 미만 단일 기준 | project_year | 270 | 179 | 2.0% | STRONG_SIGNAL | HIGH_FOR_ABSOLUTE_RULE |
| 집행률 90% 미만 단일 기준 | project_year | 532 | 303 | 11.9% | SENSITIVITY_OR_SUMMARY | HIGH_FOR_ABSOLUTE_RULE |
| 80%·90% 2단계 기준 | project_year | 532 | 303 | 11.9% | PRIMARY_POLICY | HIGH_FOR_ABSOLUTE_RULE |
| 절대 기준과 상대 기준 병행 | project_year | 710 | 394 | 12.4% | PRIMARY_PLUS_AUXILIARY | CONDITIONAL_ON_PEER_CONFIDENCE |
| 연말집중 고정 기준 | project_year | 461 | 349 | 6.4% | PRIMARY_EXPLORATION | MEDIUM |
| 고정 기준과 보수적 P90 병행 | project_year | 552 | 421 | 15.4% | PRIMARY_PLUS_AUXILIARY | CONDITIONAL_ON_PEER_CONFIDENCE |
| 반복 2회 이상 | project | 565 | 220 | 6.5% | SENSITIVITY | LOW_WHEN_VALID_YEAR_COUNT_IS_SMALL |
| 반복 2회 이상 및 유효연도 50% 이상 | project | 565 | 220 | 6.5% | PRIMARY_RECURRENCE | MEDIUM_TO_HIGH_BY_VALID_YEAR_COUNT |
| 연속 2회 강화 신호 | project | 528 | 203 | 6.5% | REINFORCED_SIGNAL | MEDIUM |
...
80%에서 90%로 완화하면 탐지 행은 262행, 고유 사업은 124개 늘어납니다. 이는 강한 신호의 확대가 아니라 주의 신호를 추가하는 변화로 해석해야 합니다.
...
- **입력·분석 단위 확인** — 완료: M3 6,290행 보존, 주 ECDF 6,198행
- **집행률 분포·민감도** — 완료: ECDF 29,546행, 70~95% 민감도 468행
- **집단 편향·상대 안정성** — 완료: 편향 832행, 상대 진단 238행
- **연말집중·반복 신호** — 완료: 월별 점 3,293행, 사업-신호 반복 4,186행
