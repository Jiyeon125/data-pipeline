...
집행률 분석 적격·100% 초과 제외 모집단은 6,114행입니다. 80% 미만은 254행(4.2%), 90% 미만은 513행(8.4%)입니다. 80%와 90%는 분포가 자동으로 발견한 절단점이 아니라, 탐지 강도를 두 단계로 설명하기 위한 잠정 운영 기준입니다.
...
80% 기준은 고유 사업 169개와 예산현액 2.1%를 탐지합니다. 90% 기준은 고유 사업 292개와 예산현액 9.7%를 탐지합니다. 두 값을 합쳐 하나의 정책 판정으로 쓰지 않고 강한 신호와 주의 신호로 나누는 이유입니다.
...
89%에서 90%로 이동할 때 새로 포함되는 행은 40개지만 예산현액 비중은 6.4%p 증가합니다. 90% 기준의 금액 영향은 소수 대규모 사업에 민감하므로 해당 구간 원자료를 별도 보존했습니다.
...
현재 범위에서 90% 미만 탐지 예산현액 비중은 9.7%이며, 보통교부세를 민감도에서 제외하면 4.6%입니다. 국채·채권매입은 범위 규칙 적용 후 현재 M3에 0행입니다.
...
|---|---|---:|---:|---:|
| ACCOUNT_TYPE | RESPONSIBLE_OPERATION_ACCOUNT | 182 | 23.1% | 5.55x |
| FISCAL_INSTRUMENT | GUARANTEE | 44 | 22.7% | 5.47x |
| DATA_QUALITY | RESTRICTED | 255 | 12.9% | 3.12x |
| FISCAL_INSTRUMENT | LOAN | 60 | 11.7% | 2.81x |
| FISCAL_INSTRUMENT | FACILITY | 141 | 10.6% | 2.56x |
...
| 차원 | 집단 | 적격 행 | 탐지율 | 전체 대비 배수 |
|---|---|---:|---:|---:|
| ACCOUNT_TYPE | RESPONSIBLE_OPERATION_ACCOUNT | 182 | 44.5% | 5.30x |
| DATA_QUALITY | RESTRICTED | 255 | 36.9% | 4.39x |
| FISCAL_INSTRUMENT | GUARANTEE | 44 | 27.3% | 3.25x |
| FISCAL_INSTRUMENT | FACILITY | 141 | 20.6% | 2.45x |
| FISCAL_INSTRUMENT | OPERATION | 684 | 19.9% | 2.37x |
| PROJECT_SIZE | Q1_SMALL | 778 | 17.7% | 2.11x |

## 4. 상대 기준은 적용 가능성과 신뢰도를 분리합니다

하위 10% 기준의 비교집단-연도 119개 중 NOT_AVAILABLE 71개, LOW 21개, MEDIUM 13개, HIGH 14개입니다.
...
검증된 월별 패턴에서 4분기 기준만 충족한 행은 77행, 12월 기준만 충족한 행은 202행, 두 기준을 모두 충족한 행은 174행입니다. 고정 기준은 충족하지 않지만 보수적 P90인 행은 90행이고, 고정 기준과 P90을 모두 충족한 행은 327행입니다.
...
동일 신호가 2회 이상이면서 유효연도의 50% 이상인 사업은 집행률 90% 미만 기준 134개, 고정 연말집중 기준 110개입니다.
...
고정 연말집중 반복 후보 110개는 현재 모두 유효 월별 관측 2년에서 연속 2회 발생한 사업입니다. 월별 관측 가능한 연도가 제한되어 있으므로 집행률 반복보다 증거가 약하며, 발표에서는 '2년 연속 관찰'로 표현해야 합니다.
...
|---|---|---:|---:|---:|---|---|
| 집행률 80% 미만 단일 기준 | project_year | 254 | 169 | 2.1% | STRONG_SIGNAL | HIGH_FOR_ABSOLUTE_RULE |
| 집행률 90% 미만 단일 기준 | project_year | 513 | 292 | 9.7% | SENSITIVITY_OR_SUMMARY | HIGH_FOR_ABSOLUTE_RULE |
| 80%·90% 2단계 기준 | project_year | 513 | 292 | 9.7% | PRIMARY_POLICY | HIGH_FOR_ABSOLUTE_RULE |
| 절대 기준과 상대 기준 병행 | project_year | 701 | 390 | 11.2% | PRIMARY_PLUS_AUXILIARY | CONDITIONAL_ON_PEER_CONFIDENCE |
| 연말집중 고정 기준 | project_year | 453 | 343 | 8.5% | PRIMARY_EXPLORATION | MEDIUM |
| 고정 기준과 보수적 P90 병행 | project_year | 543 | 414 | 8.8% | PRIMARY_PLUS_AUXILIARY | CONDITIONAL_ON_PEER_CONFIDENCE |
| 반복 2회 이상 | project | 546 | 213 | 9.0% | SENSITIVITY | LOW_WHEN_VALID_YEAR_COUNT_IS_SMALL |
| 반복 2회 이상 및 유효연도 50% 이상 | project | 546 | 213 | 9.0% | PRIMARY_RECURRENCE | MEDIUM_TO_HIGH_BY_VALID_YEAR_COUNT |
| 연속 2회 강화 신호 | project | 509 | 196 | 9.0% | REINFORCED_SIGNAL | MEDIUM |
...
80%에서 90%로 완화하면 탐지 행은 259행, 고유 사업은 123개 늘어납니다. 이는 강한 신호의 확대가 아니라 주의 신호를 추가하는 변화로 해석해야 합니다.

절대 기준과 보수적 상대 기준을 병행하면 탐지 고유 사업 비율은 19.1%입니다. 다만 상대 신호 LOW 집단까지 동일하게 강조하면 비교집단 소표본이 다시 결과를 지배할 수 있습니다.
...
- **입력·분석 단위 확인** — 완료: M3 6,130행 보존, 주 ECDF 6,114행
- **집행률 분포·민감도** — 완료: ECDF 29,234행, 70~95% 민감도 468행
- **집단 편향·상대 안정성** — 완료: 편향 832행, 상대 진단 238행
- **연말집중·반복 신호** — 완료: 월별 점 3,247행, 사업-신호 반복 4,086행
