...
집행률 분석 적격·100% 초과 제외 모집단은 4,621행입니다. 80% 미만은 198행(4.3%), 90% 미만은 414행(9.0%)입니다. 80%와 90%는 분포가 자동으로 발견한 절단점이 아니라, 탐지 강도를 두 단계로 설명하기 위한 잠정 운영 기준입니다.
...
80% 기준은 고유 사업 134개와 예산현액 2.3%를 탐지합니다. 90% 기준은 고유 사업 232개와 예산현액 13.6%를 탐지합니다. 두 값을 합쳐 하나의 정책 판정으로 쓰지 않고 강한 신호와 주의 신호로 나누는 이유입니다.
...
89%에서 90%로 이동할 때 새로 포함되는 행은 31개지만 예산현액 비중은 9.6%p 증가합니다. 90% 기준의 금액 영향은 소수 대규모 사업에 민감하므로 해당 구간 원자료를 별도 보존했습니다.

| 부처 | 프로그램 | 세부사업 | 연도 | 집행률 | 예산현액 |
|---|---|---|---:|---:|---:|
| 행정안전부 | 지방교부세 | 보통교부세 | 2023 | 89.2% | 66.64조원 |
| 행정안전부 | 전자정부 | 국가정보관리원대구센터신축(정보화) | 2023 | 89.9% | 0.09조원 |
| 과학기술정보통신부 | 과학기술기반조성 | 국제과학비즈니스벨트조성(R&D) | 2024 | 89.4% | 0.06조원 |
| 보건복지부 | 보건의료서비스지원 | 의료기술시험연수원 구축 | 2025 | 89.6% | 0.06조원 |
| 중소벤처기업부 | 중소기업인력지원 | 마이스터고육성 | 2022 | 89.9% | 0.03조원 |

예산현액 상위 두 행 가운데 재정수단 `UNKNOWN`은 1행, 현재 분석 포함 분류는 2행입니다. 따라서 90% 기준의 금액 비중을 발표하기 전에는 이 대규모 사업들의 정책사업 범위와 재정수단을 먼저 확인해야 합니다. 이 표는 기준을 폐기하는 근거가 아니라 금액가중 결과가 분류 검토에 민감하다는 근거입니다.

현재 범위에서 90% 미만 탐지 예산현액 비중은 13.6%이며, 보통교부세를 민감도에서 제외하면 6.4%입니다. 국채·채권매입은 범위 규칙 적용 후 현재 M3에 0행입니다.
...
|---|---|---:|---:|---:|
| ACCOUNT_TYPE | RESPONSIBLE_OPERATION_ACCOUNT | 182 | 23.1% | 5.39x |
| DATA_QUALITY | RESTRICTED | 254 | 13.0% | 3.03x |
| FISCAL_INSTRUMENT | OPERATION | 515 | 12.2% | 2.85x |
| FISCAL_INSTRUMENT | FACILITY | 117 | 10.3% | 2.39x |
| PROJECT_SIZE | Q1_SMALL | 652 | 9.4% | 2.18x |
| MINISTRY | 중소벤처기업부 | 366 | 1.1% | 0.26x |

### 90% 기준에서 우선 확인할 집단

| 차원 | 집단 | 적격 행 | 탐지율 | 전체 대비 배수 |
|---|---|---:|---:|---:|
| ACCOUNT_TYPE | RESPONSIBLE_OPERATION_ACCOUNT | 182 | 44.5% | 4.97x |
| DATA_QUALITY | RESTRICTED | 254 | 37.0% | 4.13x |
| FISCAL_INSTRUMENT | OPERATION | 515 | 22.9% | 2.56x |
| FISCAL_INSTRUMENT | FACILITY | 117 | 22.2% | 2.48x |
| PROJECT_SIZE | Q1_SMALL | 652 | 18.7% | 2.09x |
| MINISTRY | 과학기술정보통신부 | 1,781 | 4.4% | 0.49x |

## 4. 상대 기준은 적용 가능성과 신뢰도를 분리합니다

하위 10% 기준의 비교집단-연도 81개 중 NOT_AVAILABLE 49개, LOW 12개, MEDIUM 10개, HIGH 10개입니다.
...
검증된 월별 패턴에서 4분기 기준만 충족한 행은 42행, 12월 기준만 충족한 행은 127행, 두 기준을 모두 충족한 행은 139행입니다. 고정 기준은 충족하지 않지만 보수적 P90인 행은 74행이고, 고정 기준과 P90을 모두 충족한 행은 233행입니다.
...
동일 신호가 2회 이상이면서 유효연도의 50% 이상인 사업은 집행률 90% 미만 기준 110개, 고정 연말집중 기준 78개입니다.
...
고정 연말집중 반복 후보 78개는 현재 모두 유효 월별 관측 2년에서 연속 2회 발생한 사업입니다. 월별 관측 가능한 연도가 제한되어 있으므로 집행률 반복보다 증거가 약하며, 발표에서는 '2년 연속 관찰'로 표현해야 합니다.
...
|---|---|---:|---:|---:|---|---|
| 집행률 80% 미만 단일 기준 | project_year | 198 | 134 | 2.3% | STRONG_SIGNAL | HIGH_FOR_ABSOLUTE_RULE |
| 집행률 90% 미만 단일 기준 | project_year | 414 | 232 | 13.6% | SENSITIVITY_OR_SUMMARY | HIGH_FOR_ABSOLUTE_RULE |
| 80%·90% 2단계 기준 | project_year | 414 | 232 | 13.6% | PRIMARY_POLICY | HIGH_FOR_ABSOLUTE_RULE |
| 절대 기준과 상대 기준 병행 | project_year | 552 | 305 | 14.2% | PRIMARY_PLUS_AUXILIARY | CONDITIONAL_ON_PEER_CONFIDENCE |
| 연말집중 고정 기준 | project_year | 308 | 230 | 10.8% | PRIMARY_EXPLORATION | MEDIUM |
| 고정 기준과 보수적 P90 병행 | project_year | 382 | 288 | 11.2% | PRIMARY_PLUS_AUXILIARY | CONDITIONAL_ON_PEER_CONFIDENCE |
| 반복 2회 이상 | project | 429 | 164 | 7.5% | SENSITIVITY | LOW_WHEN_VALID_YEAR_COUNT_IS_SMALL |
| 반복 2회 이상 및 유효연도 50% 이상 | project | 429 | 164 | 7.5% | PRIMARY_RECURRENCE | MEDIUM_TO_HIGH_BY_VALID_YEAR_COUNT |
| 연속 2회 강화 신호 | project | 392 | 147 | 7.5% | REINFORCED_SIGNAL | MEDIUM |
...
80%에서 90%로 완화하면 탐지 행은 216행, 고유 사업은 98개 늘어납니다. 이는 강한 신호의 확대가 아니라 주의 신호를 추가하는 변화로 해석해야 합니다.

절대 기준과 보수적 상대 기준을 병행하면 탐지 고유 사업 비율은 19.5%입니다. 다만 상대 신호 LOW 집단까지 동일하게 강조하면 비교집단 소표본이 다시 결과를 지배할 수 있습니다.
...
- **입력·분석 단위 확인** — 완료: M3 6,130행 보존, 주 ECDF 4,621행
- **집행률 분포·민감도** — 완료: ECDF 22,428행, 70~95% 민감도 442행
- **집단 편향·상대 안정성** — 완료: 편향 754행, 상대 진단 162행
- **연말집중·반복 신호** — 완료: 월별 점 2,466행, 사업-신호 반복 4,086행
