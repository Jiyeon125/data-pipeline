...
읽습니다. `full_population_review_work_queue.csv`는 412개 프로그램-연도-회계유형 분석행을
여섯 점검강도에 빠짐없이 배치하는 프로그램-연도-회계유형 감사 산출물입니다.
`program_year_review_queue.csv`는 회계유형 금액을 합산하고 총집행률·프로그램
총액 전년비·반복 신호를 다시 계산한 프로그램-연도 최종 점검대기열입니다. 회계행
등급의 최악값·평균값·최빈값은 사용하지 않고 같은 질문형 등급 함수를 집계 신호에
재적용합니다. 선호 키 `ministry_code + fiscal_year + program_code`에서 분야·부문·
프로그램명이 충돌하면 자동 병합하지 않고 별도 안정 ID와 H 보류로 남깁니다.
`full_population_project_review_queue.csv`는
...
- 부처·기준연도·점검등급 필터와 선택연도 프로그램 1행 계약
- 프로그램 연도별 등급·진단 타임라인과 선택연도 회계유형 원시행 드릴다운
