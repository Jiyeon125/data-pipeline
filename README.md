...
`.env`에 발급받은 인증키를 입력합니다. API 요청주소는 `configs/datasets.yaml`에서 관리합니다.
...
openfiscal smoke-test --dataset-id expenditure_budget_init
```

성공하면 인증키나 전체 응답값을 출력하지 않고 데이터셋 ID, 요청시각, 요청주소, 최상위 JSON 키만 표시합니다.

## 시험 수집

`configs/datasets.yaml`에서 `source_type: api`이고 `enabled: true`인 데이터셋을 순회 수집합니다.

```bash
openfiscal collect --max-pages 1
openfiscal collect --max-pages 1 --dataset-id expenditure_budget_init
```
...
데이터셋마다 파라미터명과 응답 필드가 다를 수 있으므로, 열린재정 Open API 상세화면의 `요청주소`, `검색 요청인자`, `출력값 명세`를 확인한 뒤 `configs/datasets.yaml`에 반영합니다.
